...
import { getBirthChartFromProfile, hasBirthChart, BirthChartData } from '../../utils/astrology/birthChart';
...
const generatePersonalizedHoroscope = (
  ...
  const risingSign = birthChart.find(p => p.body === 'Ascendant')?.sign || sunSign;
  ...
  horoscopeElements.push(`${dayOfWeek} carries the energy of ${dayOfWeekEnergy}, while today's numerological vibration of ${numerologyInfluence.number} brings ${numerologyInfluence.meaning} to your ${sunSign} Sun nature.`);
  
  // Sun transit influence - Core energy
  if (transitSun) {
    if (transitSun.sign === sunSign) {
      horoscopeElements.push(`With the Sun illuminating your natal sign, this is your season of maximum vitality and authentic self-expression. Your natural ${sunSign} qualities are magnified, making this an ideal time to step into leadership roles and pursue goals that align with your core identity.`);
    } else {
      horoscopeElements.push(`The Sun's journey through ${transitSun.sign} casts a spotlight on ${getSunTransitMeaning(transitSun.sign, sunSign)}, inviting you to expand beyond your usual ${sunSign} approach. This transit encourages growth in areas that may feel unfamiliar but ultimately strengthen your character.`);
    }
  }
  ...
  if (transitMoon) {
    if (transitMoon.sign === moonSign) {
      horoscopeElements.push(`The Moon's return to your natal ${moonSign} creates an emotionally familiar and comfortable atmosphere. Your instinctive responses are heightened, and you may find yourself naturally gravitating toward activities that nurture your soul. Trust these intuitive pulls.`);
    } else {
      horoscopeElements.push(`Today's ${transitMoon.sign} Moon ${getMoonInfluence(transitMoon.sign)}, offering a complementary energy to your natal ${moonSign} Moon. This creates an opportunity to experience emotions through a different lens, potentially revealing new facets of your inner world.`);
    }
  }
  ...
  if (primaryAspect) {
    const birthPlanetSign = getBirthPlanetSign(primaryAspect.natalPlanet, birthChart);
    horoscopeElements.push(`Today's most significant cosmic influence is ${primaryAspect.transitPlanet} in ${getCurrentTransitSign(primaryAspect.transitPlanet, currentTransits)} ${getAspectDescription(primaryAspect)} your birth ${primaryAspect.natalPlanet} in ${birthPlanetSign}. ${getDetailedAspectMeaning(primaryAspect)} This creates a powerful focus on ${getPlanetTheme(primaryAspect.natalPlanet)} in your life.`);
  }
  
  // Secondary aspect - Additional influence  
  const secondaryAspect = aspects[1];
  if (secondaryAspect) {
    const birthPlanetSign = getBirthPlanetSign(secondaryAspect.natalPlanet, birthChart);
    horoscopeElements.push(`A secondary influence comes from ${secondaryAspect.transitPlanet} ${getAspectDescription(secondaryAspect)} your birth ${secondaryAspect.natalPlanet} in ${birthPlanetSign}, adding ${getAspectQuality(secondaryAspect.type)} energy to ${getPlanetTheme(secondaryAspect.natalPlanet)} matters.`);
  }
  ...
  if (transitMercury) {
    ...
    if (mercuryAspect) {
      horoscopeElements.push(`Mercury's current position particularly emphasizes ${getMercuryGuidance(transitMercury.sign)}, while its connection to your natal chart suggests this is an especially significant day for mental clarity and purposeful communication.`);
    } else {
      horoscopeElements.push(`With Mercury in ${transitMercury.sign}, the intellectual atmosphere supports ${getMercuryGuidance(transitMercury.sign)}. Consider how this mental energy can serve your broader ${sunSign} goals.`);
    }
  }
  ...
  if (transitVenus) {
    ...
    if (venusAspect) {
      const birthPlanetSign = getBirthPlanetSign(venusAspect.natalPlanet, birthChart);
      horoscopeElements.push(`Venus in ${transitVenus.sign} forms a ${venusAspect.type} aspect with your birth ${venusAspect.natalPlanet} in ${birthPlanetSign}, bringing focus to ${getVenusGuidance(transitVenus.sign)}. This alignment highlights ${getPlanetTheme(venusAspect.natalPlanet)} through the lens of love, beauty, and values.`);
    }
  }
  ...
  if (transitJupiter) {
    ...
    if (jupiterAspect) {
      horoscopeElements.push(`Jupiter's expansive influence touches your chart today, opening doorways for philosophical growth and broader perspectives. This benefic energy supports your journey toward greater wisdom and understanding.`);
    }
  }
  ...
  if (userBirthday) {
    ...
    horoscopeElements.push(`On a personal numerological level, you're experiencing a ${personalDay.number} day, making this ${personalDay.meaning}. This combines powerfully with your ${sunSign} solar energy.`);
  }
  
  // Practical guidance based on aspects
  if (aspects.length > 0) {
    const hasChallengingAspects = aspects.some(a => a.type === 'square' || a.type === 'opposition');
    const hasHarmoniousAspects = aspects.some(a => a.type === 'trine' || a.type === 'sextile');
    
    if (hasChallengingAspects && hasHarmoniousAspects) {
      horoscopeElements.push(`Today presents a balanced mix of supportive and challenging energies. Navigate any tensions with patience while embracing opportunities that align with your values. Your ${sunSign} nature provides the wisdom to discern between necessary action and patient observation.`);
    } else if (hasChallengingAspects) {
      horoscopeElements.push(`While today's aspects may present some resistance or require extra effort, remember that growth often emerges from constructive challenges. Your ${sunSign} strength will guide you through any obstacles toward meaningful progress.`);
    } else if (hasHarmoniousAspects) {
      horoscopeElements.push(`The harmonious flow of today's planetary energies creates favorable conditions for advancement in personal and professional spheres. This supportive cosmic climate enhances your natural ${sunSign} abilities.`);
    }
  }
...
const calculateKeyAspects = (birthChart: BirthChartData[], currentTransits: any[]) => {
  const aspects = [];
  ...
  for (const transit of currentTransits) {
    for (const natal of birthChart) {
      ...
      } else if (Math.abs(diff - 120) <= 8) { // Trine
        aspects.push({
          type: 'trine',
          transitPlanet: transit.body,
          natalPlanet: natal.body,
      ...
      } else if (Math.abs(diff - 90) <= 8) { // Square
        aspects.push({
          type: 'square',
          transitPlanet: transit.body,
          natalPlanet: natal.body,
...
const getSunTransitMeaning = (transitSign: string, natalSunSign: string): string => {
...
const getAspectDescription = (aspect: any): string => {
  const orbDescription = aspect.orb <= 3 ? 'closely' : aspect.orb <= 6 ? 'moderately' : 'loosely';
  ...
  return descriptions[aspect.type] || `aspects (at ${Math.round(aspect.orb)}° orb)`;
};
...
const getDetailedAspectMeaning = (aspect: any): string => {
  const meanings: Record<string, string> = {
    'conjunction': 'When planets are in the same area of the sky, their energies blend and amplify each other.',
    'opposition': 'When planets are across the sky from each other, they create tension that requires balance.',
    'trine': 'When planets are 120° apart, they create easy, flowing energy that supports natural talents.',
    'square': 'When planets are 90° apart, they create friction that pushes you to grow and take action.',
    'sextile': 'When planets are 60° apart, they offer helpful opportunities and cooperative energy.'
  };
  
  return meanings[aspect.type] || 'This planetary relationship brings significant influence.';
};
...
const getBirthPlanetSign = (planetName: string, birthChart: BirthChartData[]): string => {
  const planet = birthChart.find(p => p.body === planetName);
  return planet?.sign || '';
};

const getRetrogradeInfluences = (currentTransits: any[], birthChart: BirthChartData[]): string[] => {
  const influences: string[] = [];
  
  // Check for currently retrograde planets
  const retrogradeTransits = currentTransits.filter(transit => transit.retrograde);
  ...
  retrogradeTransits.forEach(transit => {
    ...
    const aspects = birthChart.filter(birthPlanet => {
      let diff = Math.abs(transit.eclipticLongitude - birthPlanet.eclipticLongitude);
      if (diff > 180) diff = 360 - diff;
      return Math.abs(diff - 0) <= 10 || Math.abs(diff - 180) <= 10 || Math.abs(diff - 120) <= 8 || Math.abs(diff - 90) <= 8;
    });
    
    if (aspects.length > 0) {
      const retroMessage = getRetrogradeMessage(transit.body, transit.sign);
      influences.push(`${transit.body} is currently retrograde in ${transit.sign}, ${retroMessage}`);
    } else {
      // General retrograde influence
      const generalMessage = getGeneralRetrogradeMessage(transit.body, transit.sign);
...
const getRetrogradeMessage = (planet: string, sign: string): string => {
  ...
  return messages[planet] || `encouraging reflection and internal processing of ${planet.toLowerCase()} themes`;
};
...
const getGeneralRetrogradeMessage = (planet: string, sign: string): string => {
  ...
  return general[planet] || `${planet} retrograde in ${sign} brings introspective energy to the day.`;
};

const getDailyNumerology = (date: dayjs.Dayjs): { number: number; meaning: string } => {
...
const getPersonalDayNumber = (birthdate: string, currentDate: dayjs.Dayjs): { number: number; meaning: string } => {
...
const getAspectInterpretation = (aspect: any): string => {
  ...
  return interpretations[type][aspectKey] || 
...
export const HoroscopeWidget = () => {
  ...
  const birthChart = hasBirthChartData ? getBirthChartFromProfile(me.profile) : null;
  ...
  if (!birthChart) {
    return (
      <div className='py-3 px-4 border border-stone-800 rounded-md w-full'>
        <div className='text-center'>
          <h3 className='font-bold mb-2'>Daily Horoscope</h3>
          <p className='text-zinc-400 text-xs'>Calculating your cosmic influences...</p>
        </div>
      </div>
    );
  }
  ...
  const horoscope = generatePersonalizedHoroscope(birthChart, currentTransits, userName, userBirthday);
    ...
    <div className='py-3 px-4 border border-stone-800 rounded-md w-full'>
      ...
      <div className='flex justify-center items-center gap-3 mb-3'>
        {primaryTransit && (
          <div className='text-center'>
            <div className='text-lg mb-1'>
              {bodiesSymbols[primaryTransit.body.toLowerCase() as keyof typeof bodiesSymbols]}
            </div>
        ...
        {moonTransit && (
          <div className='text-center'>
            <div className='text-lg mb-1'>
              {bodiesSymbols[moonTransit.body.toLowerCase() as keyof typeof bodiesSymbols]}
            </div>
            <div className='text-xs text-zinc-400'>
              {moonTransit.sign}
            </div>
