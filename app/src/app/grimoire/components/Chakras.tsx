    ...
    <div>
      ...
      {chakraList.map((chakra: string) => (
        <div key={chakra} className="mb-3">
          <h2 className='font-bold pb-1'>{chakras[chakra as keyof typeof chakras].symbol} {chakras[chakra as keyof typeof chakras].name}</h2>
          {/* <p>{chakras[chakra as keyof typeof chakras].meaning}</p> */}
          <p>{chakras[chakra as keyof typeof chakras].properties}</p>
          <p className='pt-1'>{chakras[chakra as keyof typeof chakras].mysticalProperties}</p>
        </div>
      ))}
    </div>
  );
