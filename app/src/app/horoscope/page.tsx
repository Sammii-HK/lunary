...
import { getUpcomingTransits, getSolarReturnInsights, TransitEvent } from '../../../utils/astrology/transitCalendar';
...
export default function HoroscopePage() {
  ...
  const horoscope = getEnhancedPersonalizedHoroscope(userBirthday, userName, me?.profile);
  ...
  const solarReturnData = userBirthday ? getSolarReturnInsights(userBirthday) : null;
  ...
  return (
    <div className='h-[91vh] space-y-6 pb-4 px-4'>
      ...
      <div className='bg-gradient-to-r from-purple-900/40 to-blue-900/40 rounded-lg p-4 border border-purple-700'>
        <h2 className='text-lg font-semibold text-purple-300 mb-3'>✨ Cosmic Highlight</h2>
        <p className='text-purple-100 leading-relaxed'>{horoscope.cosmicHighlight}</p>
      </div>
      ...
      <div className='bg-gradient-to-r from-yellow-900/40 to-orange-900/40 rounded-lg p-4 border border-yellow-700'>
        <h2 className='text-lg font-semibold text-yellow-300 mb-3'>🌟 Daily Affirmation</h2>
        <p className='text-yellow-100 leading-relaxed italic text-center'>"{horoscope.dailyAffirmation}"</p>
      </div>
      ...
      <div className='bg-zinc-800 rounded-lg p-4'>
        ...
        <p className='text-sm text-zinc-400 mb-3'>
          Personalized daily elements based on planetary positions, numerology, and your birth chart
        </p>
      ...
      {solarReturnData && (
        <div className='bg-gradient-to-r from-orange-900/40 to-red-900/40 rounded-lg p-4 border border-orange-700'>
          <h2 className='text-lg font-semibold text-orange-300 mb-3'>🌅 Solar Return Insights</h2>
          <div className='space-y-3'>
            ...
            <div className='flex justify-between items-center'>
              ...
              <span className='text-orange-200 font-medium'>{solarReturnData.personalYear}</span>
            </div>
            <p className='text-orange-100 leading-relaxed text-sm'>{solarReturnData.insights}</p>
            <div>
              <h4 className='text-sm font-medium text-orange-200 mb-2'>Year Themes:</h4>
              <div className='flex flex-wrap gap-2'>
                {solarReturnData.themes.map((theme, index) => (
                  <span key={index} className='bg-orange-800/40 text-orange-200 px-2 py-1 rounded text-xs'>
                    {theme}
                  </span>
                ))}
              </div>
      ...
      {/* Transit Calendar */}
      <div className='bg-zinc-800 rounded-lg p-4'>
        <h2 className='text-lg font-semibold text-indigo-400 mb-3'>🌌 Transit Calendar</h2>
        <p className='text-sm text-zinc-400 mb-4'>
          Upcoming planetary events and moon phases over the next 30 days
        </p>
        <div className='space-y-3 max-h-96 overflow-y-auto'>
          {upcomingTransits.slice(0, 10).map((transit: TransitEvent, index: number) => (
            <div key={index} className='bg-zinc-700 rounded p-3 border-l-4 border-indigo-500'>
              <div className='flex justify-between items-start mb-2'>
                ...
                <span className={`px-2 py-1 rounded text-xs font-medium ${
                  transit.significance === 'high' ? 'bg-red-900/40 text-red-300' :
                  transit.significance === 'medium' ? 'bg-yellow-900/40 text-yellow-300' :
                  'bg-blue-900/40 text-blue-300'
                }`}>
                  {transit.significance}
                </span>
              </div>
              <p className='text-sm text-zinc-300 leading-relaxed'>{transit.description}</p>
            </div>
          ))}
          {upcomingTransits.length === 0 && (
            <p className='text-zinc-400 text-center py-4'>No significant transits in the next 30 days</p>
          )}
        </div>
      </div>
