# Competitor Comparison Page Guidelines

## ✅ **Legal & Ethical Best Practices**

### 1. **Naming Competitors Directly**

**✅ YES, it's legal and common** to name competitors in comparison pages, BUT:

**Do:**

- ✅ Use factual, verifiable information
- ✅ Focus on YOUR features and benefits
- ✅ Be fair and accurate
- ✅ Include disclaimers
- ✅ Use "vs" format (standard SEO practice)
- ✅ Focus on helping users make informed decisions

**Don't:**

- ❌ Make false or defamatory statements
- ❌ Use misleading comparisons
- ❌ Attack competitors personally
- ❌ Make unsubstantiated claims
- ❌ Use competitor trademarks incorrectly

### 2. **What Makes a Good Comparison Page**

**Structure:**

1. **Fair Introduction** - Acknowledge both options
2. **Feature Comparison Table** - Side-by-side factual comparison
3. **Your Advantages** - Focus on what YOU do well
4. **When Competitor Might Be Better** - Shows fairness
5. **Conclusion** - Helpful summary
6. **Legal Disclaimer** - Protects you

**Tone:**

- Professional and helpful
- Factual, not emotional
- Focus on features, not attacks
- Help users make informed decisions

### 3. **SEO Benefits**

**Why Comparison Pages Work:**

- ✅ Target high-intent keywords ("X vs Y")
- ✅ Answer user questions directly
- ✅ Build authority and trust
- ✅ High conversion rates (users comparing are ready to buy)
- ✅ Natural backlink opportunities

**Example Keywords:**

- "lunary vs moonly"
- "best personalized astrology app"
- "lunary vs lunar guide"
- "astrology app comparison"

### 4. **Legal Protection**

**Include Disclaimers:**

- "Based on publicly available information"
- "Features may change"
- "Not intended to disparage competitors"
- "For informational purposes only"
- "Competitor names are trademarks of their owners"

**Why This Works:**

- Shows good faith
- Protects against legal issues
- Maintains professional tone
- Standard practice in industry

### 5. **Examples of Good Comparison Pages**

**Companies That Do This Well:**

- HubSpot vs competitors
- Shopify vs competitors
- Canva vs competitors
- All major SaaS companies

**Why They Work:**

- Factual comparisons
- Focus on their strengths
- Help users decide
- Fair and professional

## 🎯 **Our Strategy**

### Pages to Create:

1. **"Lunary vs Moonly"** ✅ Created

   - Target: "lunary vs moonly", "moonly alternative"
   - Focus: Real astronomy, personalized charts, grimoire

2. **"Lunary vs Lunar Guide"**

   - Target: "lunary vs lunar guide", "lunar guide alternative"
   - Focus: Real astronomy vs AI-powered generic astrology

3. **"Best Personalized Astrology Apps"**

   - Target: "best personalized astrology app", "astrology app comparison"
   - Include: Lunary, Moonly, Lunar Guide, others
   - Position Lunary as #1 choice

4. **"Lunary vs Generic Astrology Apps"**
   - Target: "personalized astrology vs generic"
   - Focus: Why personalized matters

### Content Strategy:

**Each Page Should:**

- ✅ Compare 3-5 key features
- ✅ Highlight Lunary's unique advantages
- ✅ Acknowledge competitor strengths fairly
- ✅ Include clear CTAs
- ✅ Have proper disclaimers
- ✅ Target specific keywords

**SEO Optimization:**

- ✅ Target "X vs Y" keywords
- ✅ Include competitor name in title
- ✅ Use structured data (ComparisonPage schema)
- ✅ Internal links to relevant pages
- ✅ Share on social media

## 📊 **Expected Results**

**Short-term (1-2 months):**

- Rank for "lunary vs [competitor]" searches
- Capture comparison traffic
- Higher conversion rates (comparison = high intent)

**Long-term (3-6 months):**

- Build authority in comparison searches
- Natural backlinks from comparison sites
- Dominate "best astrology app" searches

## ⚖️ **Legal Safety**

**You're Protected If You:**

- ✅ Use factual information
- ✅ Focus on your features
- ✅ Include disclaimers
- ✅ Don't make false claims
- ✅ Use competitor names fairly

**This is Standard Practice:**

- All major companies do this
- Legal and ethical when done right
- Helps users make informed decisions
- Standard SEO practice

## 🚀 **Next Steps**

1. ✅ Created "Lunary vs Moonly" page
2. ⚠️ Create "Lunary vs Lunar Guide" page
3. ⚠️ Create "Best Personalized Astrology Apps" hub page
4. ⚠️ Add ComparisonPage structured data
5. ⚠️ Link from homepage and pricing page
6. ⚠️ Share on social media
