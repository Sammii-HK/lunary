 ...
 */
export async function hasCompletedTour(userId: string, tourId: TourId): Promise<boolean> {
...
export async function getUserTourProgress(userId: string) {
  ...
  return {
    completed: progress.filter((p) => p.status === 'COMPLETED').map((p) => p.tourId),
    dismissed: progress.filter((p) => p.status === 'DISMISSED').map((p) => p.tourId),
    lastShownAt: progress.reduce((acc, p) => {
 ...
 */
export function createHasSeenTourFn(completedTours: string[], dismissedTours: string[]) {
