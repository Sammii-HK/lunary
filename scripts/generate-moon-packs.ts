...
class MoonPackGenerator {
  ...
  async generatePacks(type: 'monthly' | 'quarterly' | 'yearly' | 'all' = 'all') {
    console.log(`🌙 Starting moon pack generation (${type})${this.dryRun ? ' [DRY RUN]' : ''}`);
  ...
  private async generateMonthlyPacks(currentYear: number, currentMonth: number) {
  ...
  private async generateQuarterlyPacks(currentYear: number, currentQuarter: number) {
  ...
  private async generateYearlyPacks(currentYear: number) {
    ...
    if (currentMonth >= 7) {
      ...
      console.log(`⏰ Skipping yearly pack generation - too early in year (current month: ${currentMonth})`);
    }
  }
  ...
  async cleanup() {
    ...
    try {
      ...
      console.log(`   ✅ Cleanup completed. ${deactivatedCount} old packs ${this.dryRun ? 'would be' : ''} deactivated.`);
...
async function main() {
  ...
  const type = typeArg ? typeArg.split('=')[1] as 'monthly' | 'quarterly' | 'yearly' | 'all' : 'all';
