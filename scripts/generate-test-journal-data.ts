...
/**
 * Generate realistic test journal data for pattern detection testing
 * Creates entries with:
 * - Mood tags
 * - Moon phase data
 * - Meaningful text with house-related keywords
 * - Spread across 30 days with varied transits
 */
...
async function generateTestData() {
  ...
  let testUserId = 'test-pattern-user-001';
  ...
  // Generate 20 journal entries over the last 30 days
  const now = new Date();
  const entries: any[] = [];
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const moonPhase = await getAccurateMoonPhase(entryDate);
  ...
  console.log(
    `   Date range: ${entries[entries.length - 1].date.toISOString().split('T')[0]} to ${entries[0].date.toISOString().split('T')[0]}`,
  );
  console.log(`   Patterns embedded:`);
  console.log(`   - "reflective" during New Moon in Pisces`);
  console.log(`   - "energized" when Mars in Aries`);
  console.log(`   - Relationship themes (7th house activation)`);
  console.log(`   - Career themes (10th house activation)\n`);
  ...
  console.log(`✅ Inserted ${inserted} journal entries into database\n`);

  console.log('🎯 Next steps:');
  console.log(
    '1. Run pattern analysis: curl -X GET "http://localhost:3000/api/cron/journal-patterns" \\',
  );
  console.log('     -H "Authorization: Bearer $CRON_SECRET"');
  console.log(
    "2. Check patterns: SELECT * FROM journal_patterns WHERE user_id = 'test-pattern-user-001';",
  );
  console.log(`3. Test in chat: Ask about patterns for user ${testUserId}\n`);
