...
function canonicaliseEventType(raw: unknown): {
  ...
  // Trial countdown banner A/B test events. Map to canonical paywall
  // impressions / CTA clicks so the KPI dashboards can read them without
  // introducing new KPI event types. The legacy name is kept in metadata
  // for admin drill-down by test name.
  if (value === 'trial_countdown_view') {
    return { eventType: 'paywall_shown', legacyEventType: value };
  }
  if (value === 'trial_countdown_cta_click') {
