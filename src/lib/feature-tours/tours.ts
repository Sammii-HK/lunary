...
export const FEATURE_TOURS: FeatureTour[] = [
    ...
    steps: [
      {
        ...
        content:
          'Every morning, we update this with personalized insights from your birth chart.',
        icon: 'Sparkles',
        placement: 'center',
        action: {
          label: 'Next',
  ...
  // COLLECTIONS DISCOVERY (paid users only)
  {
    ...
    triggerCondition: (ctx) =>
      ctx.chatCount === 3 && !ctx.hasSeenTour('collections_discovery'),
    excludedTier: ['free'],
    ...
    steps: [
      {
        ...
        content:
          'Love this response? Save it to Collections and organize in custom folders.',
        icon: 'FolderOpen',
        placement: 'right',
        action: {
  ...
  // BOOK OF SHADOWS INTRO (paid users after first journal save)
  {
    ...
    trigger: 'milestone',
    triggerCondition: (ctx) =>
      ctx.journalCount === 1 && !ctx.hasSeenTour('book_of_shadows_intro'),
    excludedTier: ['free'],
    steps: [
      {
        ...
        content:
          'Track patterns, dreams, and cosmic connections over time. All your reflections in one place.',
        icon: 'BookOpen',
        placement: 'right',
        action: {
  ...
  // MOON CIRCLES (paid users, 2 days before new/full moon)
  {
    ...
    trigger: 'milestone',
    triggerCondition: (ctx) =>
      isNearMoonPhase() && !ctx.hasSeenTour('moon_circles_unlock'),
    requiredTier: ['lunary_plus', 'lunary_plus_ai', 'lunary_plus_ai_annual'],
    steps: [
      {
        ...
        content:
          'Set intentions aligned with the New Moon energy. Get guidance for this lunar cycle.',
        icon: 'Moon',
        placement: 'right',
        action: {
  ...
  // WEEKLY REPORT AVAILABLE (all users)
  {
    ...
    trigger: 'milestone',
    triggerCondition: (ctx) =>
      ctx.daysActive >= 7 && !ctx.hasSeenTour('weekly_report_available'),
    steps: [
      {
        target: '[data-nav="weekly-report"]',
        title: 'Your Weekly Cosmic Report',
        content:
          "Review your week's patterns, insights, and astrological highlights in one comprehensive report.",
        icon: 'TrendingUp',
        placement: 'right',
        action: {
          label: 'View Report',
