...
export async function generateDailyPrompt(
  ...
  try {
    ...
    parts.push(`Hello${userName ? ` ${userName}` : ''}! ${moonEmoji} Today's ${moonPhase} Moon in ${moonSign}`);
    ...
    const question = getDailyReflectionQuestion(moonPhase, moonSign, mainTransit);
    parts.push(question);
...
export async function generateWeeklyPrompt(
  ...
  try {
    ...
    const moonPhases = await getWeekMoonPhases(weekStart.toDate(), weekEnd.toDate());
    ...
    parts.push(`Hello${userName ? ` ${userName}` : ''}! 🌟 Your weekly cosmic overview`);

    // Week dates
    parts.push(`This week (${weekStart.format('MMM D')} - ${weekEnd.format('MMM D')})`);
    ...
    console.error('[Prompt Generator] Failed to generate weekly prompt:', error);
    return null;
  }
}
 ...
 */
function getLunarEvent(moon: { phase: string; sign: string } | null): string | null {
  if (!moon) return null;
...
function getDailyReflectionQuestion(
  ...
  if (phase.includes('new moon')) {
    questions.push('What intention would you like to set for this lunar cycle?');
...
export async function getOrGenerateDailyPrompt(
  ...
  try {
    ...
    console.error('[Prompt Generator] Failed to get or generate daily prompt:', error);
    return null;
  }
}
...
export async function getOrGenerateWeeklyPrompt(
  ...
  try {
    ...
    console.error('[Prompt Generator] Failed to get or generate weekly prompt:', error);
    return null;
  }
}
