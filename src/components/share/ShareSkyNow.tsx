...
interface SkyNowData {
  positions: Record<string, PlanetPosition>;
  retrogradeCount: number;
  date: string;
}

interface ShareSkyNowProps {
  compact?: boolean; // Icon-only mode for integration
}

export function ShareSkyNow({ compact = false }: ShareSkyNowProps) {
  const { user } = useUser();
  ...
  const generateCard = useCallback(async () => {
    if (!skyData) {
      setError('No sky data available');
      return;
    }
      ...
      if (!currentShareId || !currentShareUrl) {
        const response = await fetch('/api/share/sky-now', {
          ...
          body: JSON.stringify({
            ...
            retrogradeCount: skyData.retrogradeCount,
            date: skyData.date,
            format,
          }),
        });
  ...
  const handleShare = async () => {
    ...
    const file = new File([imageBlob], 'sky-now.png', {
      type: 'image/png',
    });
    ...
    if (navigator.share && navigator.canShare?.({ files: [file] })) {
      try {
        await navigator.share({
          files: [file],
          title: 'Sky Now',
          text: `Current planetary positions - ${skyData?.retrogradeCount || 0} planets retrograde`,
        });
        shareTracking.shareCompleted(user?.id, 'sky-now', 'native');
      } catch (err) {
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      shareTracking.shareCompleted(user?.id, 'sky-now', 'clipboard');
    } catch (err) {
        ...
        {!loading && !error && imageBlob && (
          <>
            ...
            <p className='mt-4 text-xs text-zinc-400 text-center'>
              Check current planetary positions at lunary.app
            </p>
          </>
        )}
