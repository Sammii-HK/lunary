...
export async function GET(request: NextRequest) {
  ...
  // Only run on 2026-04-15
  const now = new Date();
  const dateString = now.toISOString().split('T')[0];
  if (dateString !== '2026-04-15') {
  ...
  try {
    ...
    const headline = 'Sacred community at every phase';
    const bodyText =
      'Moon Circles bring together seekers at new and full moon phases. Read what others experienced, share your own insights anonymously, and build ritual together with people on the same cosmic journey.';
    const ctaLabel = 'Explore moon circles';
    const ctaHref = `${BASE_URL}/moon-circles`;
    ...
    for (const user of users.rows) {
      try {
        ...
        await sendEmail({
          ...
          subject: 'Join the cosmic gathering at every moon',
          html,
          tracking: {
            userId: user.user_id,
            notificationType: 'announcement',
            notificationId: `announcement-moon-circles-${Date.now()}-${user.user_id}`,
            utm: {
              source: 'email',
              medium: 'announcement',
              campaign: 'moon_circles',
            },
          },
        });
