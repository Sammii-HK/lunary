...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log(
        `✅ Daily transit posts: ${(dailyPostsResult as any)?.posts?.length ?? 0} posts scheduled`,
      );
    ...
    cronResults.instagramBatch = {
      skipped: true,
      reason: 'Handled by weekly-content cron',
    };
    ...
    // Instagram stories moved to dedicated /api/cron/daily-stories route
    cronResults.instagramStories = {
      skipped: true,
      reason: 'Moved to /api/cron/daily-stories',
    };
    ...
    // Monthly and quarterly product generation paused
