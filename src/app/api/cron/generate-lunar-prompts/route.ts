...
import { detectLunarEvent, generateLunarEventPrompt } from '@/lib/ai/lunar-events';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`🌟 Lunar event detected: ${lunarEvent.type} - ${lunarEvent.description}`);
    ...
    console.log(`📝 Generating lunar event prompts for ${users.rows.length} users`);
    ...
    for (const user of users.rows) {
      try {
        ...
        errorCount++;
        console.error(`❌ Error generating lunar prompt for user ${user.user_id}:`, error);
        results.push({
          userId: user.user_id,
          success: false,
