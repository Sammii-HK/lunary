...
export async function GET(request: NextRequest) {
  ...
  // Only run on 2026-04-29
  const now = new Date();
  const dateString = now.toISOString().split('T')[0];
  if (dateString !== '2026-04-29') {
  ...
  try {
    ...
    const headline = 'Deeper personal guidance';
    const bodyText =
      'Subscribers unlock house by house breakdowns of current transits affecting your life. See exactly which life area each planet is activating, with personalized guidance based on your birth chart.';
    const ctaLabel = 'Upgrade for details';
    const ctaHref = `${BASE_URL}/upgrade`;
    ...
    for (const user of users.rows) {
      try {
        ...
        await sendEmail({
          ...
          subject: 'Your transits, house by house',
          html,
          tracking: {
            userId: user.user_id,
            notificationType: 'announcement',
            notificationId: `announcement-deeper-transits-${Date.now()}-${user.user_id}`,
            utm: {
              source: 'email',
              medium: 'announcement',
              campaign: 'deeper_transits',
            },
          },
        });
