...
export async function GET(request: NextRequest) {
  ...
  // Only run on 2026-04-08
  const now = new Date();
  const dateString = now.toISOString().split('T')[0];
  if (dateString !== '2026-04-08') {
  ...
  try {
    ...
    const headline = 'Share and reward';
    const bodyText =
      'Invite friends to Lunary using your personal code. When they sign up, they get 30 days of Pro free. When they complete their first reading or ritual, you both unlock bonus rewards. You earn a week of Pro for each friend who activates.';
    const ctaLabel = 'Get your referral code';
    const ctaHref = `${BASE_URL}/referrals`;
    ...
    for (const user of users.rows) {
      try {
        ...
        await sendEmail({
          ...
          subject: 'Invite friends, earn Pro time together',
          html,
          tracking: {
            userId: user.user_id,
            notificationType: 'announcement',
            notificationId: `announcement-referrals-${Date.now()}-${user.user_id}`,
            utm: {
              source: 'email',
              medium: 'announcement',
              campaign: 'referrals',
