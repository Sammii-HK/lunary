...
function getPlanetSymbol(planetName: string): string {
  const symbols: { [key: string]: string } = {
    'Sun': 'S', 'Moon': 'R', 'Mercury': 'T', 'Venus': 'Q', 'Mars': 'U',
    'Jupiter': 'V', 'Saturn': 'W', 'Uranus': 'X', 'Neptune': 'Y'
  };
...
async function loadAstronomiconFont() {
  try {
    ...
    const fontPath = path.join(process.cwd(), 'src', 'fonts', 'Astronomicon.ttf');
...
export async function GET(request: NextRequest) {
  ...
  const themes = [
    { background: 'linear-gradient(135deg, #0a0a1a, #1a1a2e)', accent: '#b19cd9' },
    { background: 'linear-gradient(135deg, #1a1a2e, #2d3561)', accent: '#87ceeb' },
    { background: 'linear-gradient(135deg, #2c3e50, #34495e)', accent: '#dda0dd' },
    { background: 'linear-gradient(135deg, #1e2a3a, #2c3e50)', accent: '#87cefa' },
    { background: 'linear-gradient(135deg, #1a2332, #1e3c72)', accent: '#f0a0a0' },
  ];

  const theme = { ...themes[dayVariation], textShadow: '0 2px 8px rgba(0,0,0,0.8)' };
  ...
  const isAspectEvent = primaryEvent.planetA && primaryEvent.planetB && primaryEvent.aspect;
  const isAstronomicalEvent = !isAspectEvent && primaryEvent.emoji && primaryEvent.description && !primaryEvent.name.includes('Moon');
  ...
  return new ImageResponse(
    (
      ...
      >
        {/* Event Title */}
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', paddingBottom: '40px', paddingTop: '100px' }}>
          <div style={{ fontSize: '32px', fontWeight: '400', color: 'white', textAlign: 'center', letterSpacing: '0.1em', fontFamily: 'Roboto Mono' }}>
            {primaryEvent.name}
          </div>
        </div>

        {isAspectEvent ? (
          <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-around', alignItems: 'stretch', width: '100%', flex: 1, padding: '0 200px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '200px', width: '100%', height: '90%' }}>
              {/* Planet A Column */}
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', flex: 1 }}>
                <div style={{ fontSize: '42px', fontWeight: '300', color: 'white', textAlign: 'center', marginBottom: '50px' }}>
                  {(primaryEvent as any).planetA.name}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '180px', height: '180px', borderRadius: '20px', marginBottom: '70px' }}>
                  <div style={{ fontSize: '222px', color: 'white', lineHeight: '1', fontFamily: 'Astronomicon' }}>
                    {(primaryEvent as any).planetA.symbol}
                  </div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                  <div style={{ fontSize: '28px', fontWeight: '300', color: 'white', fontFamily: 'Roboto Mono', paddingBottom: '10px' }}>
                    {(primaryEvent as any).planetA.constellation}
                  </div>
                  <div style={{ fontSize: '72px', color: 'white', fontFamily: 'Astronomicon' }}>
              ...
              {/* Aspect Column */}
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-around', flex: 1, height: '400px', marginTop: '-78px' }}>
                <div style={{ fontSize: '42px', fontWeight: '300', color: 'white', textAlign: 'center', textTransform: 'capitalize', fontFamily: 'Roboto Mono' }}>
                  {(primaryEvent as any).aspect?.replace('-', ' ') || 'Conjunction'}
                </div>
                <div style={{ fontSize: '222px', color: 'white', lineHeight: '1', width: '180px', height: '180px', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Astronomicon', marginBottom: '150px', marginTop: '75px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '80px', justifyContent: 'center', flex: 1 }}>
            <div style={{ fontSize: '200px', color: 'white', lineHeight: '1' }}>{primaryEvent.emoji}</div>
        ...
        {/* Footer */}
        <div style={{ fontSize: '28px', fontWeight: '300', color: 'white', letterSpacing: '1px', marginBottom: '40px' }}>
          lunary.app
        </div>
      </div>
    ),
    {
      ...
      fonts: [
        ...
        {
          name: 'Roboto Mono',
          data: await loadGoogleFont('Roboto+Mono:wght@300;400;700', 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 /.:'),
          style: 'normal',
        },
      ],
    },
