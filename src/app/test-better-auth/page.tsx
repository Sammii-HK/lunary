...
export default function TestBetterAuthPage() {
  ...
  const testSignOut = async () => {
    ...
    try {
      ...
      setResult({ success: true, message: 'Signed out successfully', action: 'Sign Out' });
      await checkSession(); // Refresh session
    } catch (error: any) {
  ...
  const testGetSession = async () => {
    ...
    try {
      ...
      setResult({ success: false, error: error.message, action: 'Get Session' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-zinc-800 rounded-lg p-4 mb-8">
          <h2 className="text-lg font-semibold mb-2 text-zinc-200">Current Status</h2>
          {currentUser ? (
        ...
        <div className="mt-8 bg-zinc-900 rounded-lg p-6">
          ...
          <div className="space-y-2 text-sm text-zinc-400">
            ...
            <p>✅ Database migration completed (accountID, encryptedCredentials added)</p>
            <p>✅ AuthProvider wrapping JazzReactProvider</p>
            <p>⚠️ WASM crypto compatibility issues with Next.js (working on fix)</p>
          </div>
        </div>
      </div>
    </div>
