...
export default async function YearInStarsCurrentPage() {
  ...
  if (!session?.user?.id) {
    redirect('/auth?returnTo=/year-in-stars');
  }

  const userId = session.user.id;
  const year = new Date().getUTCFullYear();

  // Server-side gate so paid TTS/forecast cost isn't paid for free users.
  // 'yearly_forecast' lives on the lunary_plus_ai_annual tier.
