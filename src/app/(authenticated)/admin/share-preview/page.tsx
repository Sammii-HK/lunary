...
export default function SharePreviewPage() {
  ...
  const buildShares = () => {
    ...
    const { firstName, profile, horoscope, birthChart } = userData;
    ...
    return [
      {
        title: 'Birth Chart',
        url: `/api/og/share/birth-chart?format=${format}&name=${encodeURIComponent(firstName)}&sun=${sun}&moon=${moon}&rising=${rising}&element=${dominantElement}&modality=${dominantModality}&insight=${encodeURIComponent('Your cosmic blueprint')}`,
      },
      {
        title: 'Daily Insight',
        url: `/api/og/daily-insight?name=${encodeURIComponent(firstName)}&format=${format}&tarot=The Star&tarotKeywords=hope healing inspiration&crystal=Rose Quartz&crystalReason=Aligns with Venus&insight=${encodeURIComponent('Today brings harmonious energy')}&personalized=true`,
      },
      {
        title: 'Cosmic State',
        url: `/api/og/share/cosmic-state?shareId=demo&format=${format}`,
      ...
      },
      {
        title: 'Horoscope',
        url: horoscope
          ? `/api/og/share/horoscope?format=${format}&shareId=demo`
          : `/api/og/share/horoscope?shareId=demo&format=${format}`,
      },
    ];
  };
    ...
    <div className='min-h-screen bg-zinc-950 text-zinc-100 p-8'>
      <div className='max-w-7xl mx-auto'>
        ...
        <div className='mt-12 pt-8 border-t border-zinc-800 text-center text-sm text-zinc-500'>
          <p>
            Admin tool for QA and testing. Images use demo data and may not
            reflect real user data.
          </p>
        </div>
      </div>
    </div>
