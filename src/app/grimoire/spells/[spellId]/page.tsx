    ...
    return (
      <div className='min-h-screen flex items-center justify-center'>
        <div className='text-center'>
          ...
          <p className='text-zinc-400 mb-6'>The spell you're looking for doesn't exist.</p>
          <Link
    ...
    <div className='min-h-screen p-4 max-w-md mx-auto'>
      ...
      <div className='mb-6'>
        ...
        <div className='flex flex-wrap gap-4 text-sm'>
          ...
          <span className={`bg-zinc-800 px-3 py-1 rounded-full ${difficultyColors[spell.difficulty]}`}>
            {spell.difficulty}
          </span>
      ...
      <div className='space-y-6'>
            ...
            <div className='space-y-2 text-sm'>
              {spell.timing.moonPhase && (
                <div>
                  ...
                  <span className='text-zinc-200'>{spell.timing.moonPhase.join(', ')}</span>
                </div>
              )}
              {spell.timing.sabbat && (
                <div>
                  <span className='text-zinc-400'>Sabbat:</span>{' '}
                  <span className='text-zinc-200'>{spell.timing.sabbat.join(', ')}</span>
                </div>
              )}
              {spell.timing.planetaryDay && (
                <div>
                  <span className='text-zinc-400'>Best Days:</span>{' '}
                  <span className='text-zinc-200'>{spell.timing.planetaryDay.join(', ')}</span>
                </div>
              )}
              ...
              {spell.ingredients.map((ingredient, index) => (
                <div key={index} className='border-l-4 border-green-600 pl-4'>
                  <div className='flex justify-between items-start mb-1'>
                    <span className='font-medium text-green-300'>{ingredient.name}</span>
                    {ingredient.amount && (
                      <span className='text-sm text-zinc-400'>{ingredient.amount}</span>
                    )}
                  </div>
                  <p className='text-sm text-zinc-300 mb-1'>{ingredient.purpose}</p>
          ...
          {spell.tools.length > 0 && (
            <div className='bg-zinc-800 rounded-lg p-4'>
              <h2 className='text-xl font-semibold text-orange-400 mb-3'>Tools Needed</h2>
              <ul className='space-y-1'>
                {spell.tools.map((tool, index) => (
                  <li key={index} className='text-zinc-200 flex items-center gap-2'>
          ...
          {/* Preparation */}
          <div className='bg-zinc-800 rounded-lg p-4'>
            <h2 className='text-xl font-semibold text-indigo-400 mb-3'>Preparation</h2>
          ...
          {/* Steps */}
          <div className='bg-zinc-800 rounded-lg p-4'>
            <h2 className='text-xl font-semibold text-purple-400 mb-3'>Ritual Steps</h2>
          ...
          {/* Correspondences */}
          <div className='bg-zinc-800 rounded-lg p-4'>
            <h2 className='text-xl font-semibold text-yellow-400 mb-3'>Correspondences</h2>
                ...
                <div>
                  ...
                  <div className='flex flex-wrap gap-1'>
                    {spell.correspondences.elements.map((element, index) => (
                      <span key={index} className='bg-yellow-900/40 text-yellow-300 px-2 py-1 rounded text-xs'>
                        {element}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className='flex flex-wrap gap-1'>
                    {spell.correspondences.crystals.map((crystal, index) => (
                      <span key={index} className='bg-purple-900/40 text-purple-300 px-2 py-1 rounded text-xs'>
                        {crystal}
                      </span>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className='flex flex-wrap gap-1'>
                    {spell.correspondences.planets.map((planet, index) => (
                      <span key={index} className='bg-blue-900/40 text-blue-300 px-2 py-1 rounded text-xs'>
                        {planet}
                      </span>
                    ))}
                  </div>
          ...
          <div className='bg-red-900/20 border border-red-800 rounded-lg p-4'>
            <h2 className='text-xl font-semibold text-red-400 mb-3'>Safety & Ethics</h2>
          ...
          {spell.variations && (
            <div className='bg-zinc-800 rounded-lg p-4'>
              <h2 className='text-xl font-semibold text-cyan-400 mb-3'>Variations</h2>
              <ul className='space-y-2 text-sm'>
          ...
          {spell.history && (
            <div className='bg-zinc-800 rounded-lg p-4'>
              <h2 className='text-xl font-semibold text-amber-400 mb-3'>Historical Context</h2>
      ...
      <div className='mt-8 p-4 bg-gradient-to-r from-purple-900/30 to-pink-900/30 rounded-lg border border-purple-500/30'>
        <h3 className='text-lg font-medium text-white mb-2'>Share This Spell</h3>
