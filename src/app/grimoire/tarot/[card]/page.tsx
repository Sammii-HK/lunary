...
export default async function TarotCardPage({
  ...
  if (cardData.type === 'major') {
    ...
    const suitCards = tarotCards.minorArcana[
...
The card's meaning is enriched by its position in the spread and its relationship to other cards in the reading.`}
        numerology={cardData.type === 'major' ? `Major Arcana cards are numbered 0-21, each representing a stage in the Fool's Journey. ${cardData.card.name} represents a specific stage in this spiritual journey.` : suitInfo ? `${suitInfo.name} cards are associated with ${suitInfo.element} element and numbered 1-10, plus court cards. Each number carries numerological significance.` : ''}
        astrologyCorrespondences={cardData.type === 'major' ? `Major Arcana cards are associated with astrological signs, planets, and elements. ${cardData.card.name} connects to specific cosmic energies.` : suitInfo ? `Suit: ${suitInfo.name}
