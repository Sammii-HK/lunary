...
import {
  ASPECTS as SEO_ASPECTS,
  PLANETS as SEO_PLANETS,
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPageSources: Record<string, string[]> = {
    ...
    'grimoire/datasets/current-sky': [
      'src/app/grimoire/datasets/current-sky/route.ts',
      'src/lib/seo/citation-snapshot-store.ts',
    ],
    ...
    'grimoire/facts/moon-phase-today': [
      'src/app/grimoire/facts/moon-phase-today/page.tsx',
    ...
    'grimoire/facts/mercury-retrograde-status': [
      'src/app/grimoire/facts/mercury-retrograde-status/page.tsx',
      'src/lib/seo/current-sky-fact-pages.ts',
    ],
    'grimoire/facts/next-full-moon': [
      'src/app/grimoire/facts/next-full-moon/page.tsx',
      'src/lib/seo/astrology-calendar-dataset.ts',
    ],
    'grimoire/facts/next-new-moon': [
      'src/app/grimoire/facts/next-new-moon/page.tsx',
      'src/lib/seo/astrology-calendar-dataset.ts',
    ],
    'grimoire/facts/next-eclipse': [
      'src/app/grimoire/facts/next-eclipse/page.tsx',
      'src/lib/seo/astrology-calendar-dataset.ts',
    ],
    'grimoire/facts/next-mercury-retrograde': [
      'src/app/grimoire/facts/next-mercury-retrograde/page.tsx',
      'src/lib/seo/astrology-calendar-dataset.ts',
    ],
  ...
  const staticPageMeta: RouteConfig[] = [
    ...
    {
      path: 'grimoire/datasets/current-sky',
      changeFrequency: 'daily',
      priority: 0.6,
    },
    {
    ...
    {
      path: 'grimoire/facts/moon-phase-today',
      changeFrequency: 'daily',
      priority: 0.7,
    },
    {
    ...
    {
      path: 'grimoire/facts/planetary-positions-today',
      changeFrequency: 'daily',
      priority: 0.7,
    },
    {
      path: 'grimoire/facts/mercury-retrograde-status',
      changeFrequency: 'daily',
      priority: 0.7,
    },
    {
      path: 'grimoire/facts/next-full-moon',
      changeFrequency: 'daily',
      priority: 0.7,
    },
    {
      path: 'grimoire/facts/next-new-moon',
      changeFrequency: 'daily',
      priority: 0.7,
    },
    {
      path: 'grimoire/facts/next-eclipse',
      changeFrequency: 'daily',
      priority: 0.7,
    },
    {
      path: 'grimoire/facts/next-mercury-retrograde',
      changeFrequency: 'daily',
      priority: 0.7,
    },
    {
  ...
  const grimoireItems = Object.keys(grimoire).filter((item) =>
    RECOVERY_GRIMOIRE_SECTIONS.has(sectionToSlug(item)),
  );
  ...
  // so the sitemap does not balloon with speculative, low-demand URLs.
  ...
  const moonEndYear = Math.max(currentYear + 1, startYear + 1);
  const moonYears = Array.from(
    { length: moonEndYear - startYear + 1 },
    (_, i) => startYear + i,
  );

  // Add moon year pages (generated by moon/[year]/page.tsx).
  // Keep this aligned with generateStaticParams in that route to avoid
  // advertising unsupported future years.
  const moonYearRoutes = moonYears.map((year) => ({
  ...
  const aspectHubRoutes = SEO_PLANETS.flatMap((planet) => [
    {
      url: `${baseUrl}/grimoire/aspects/${planet}`,
      lastModified: date,
      changeFrequency: 'monthly' as const,
      priority: 0.75,
    },
    ...SEO_ASPECTS.map((aspect) => ({
      url: `${baseUrl}/grimoire/aspects/${planet}/${aspect}`,
      lastModified: date,
      changeFrequency: 'monthly' as const,
      priority: 0.7,
    })),
  ...
  // Add events year pages for the supported 2025 through current+2 window.
  ...
  // Add individual transit pages only for the active planning window.
  const transitIndividualRoutes = YEARLY_TRANSITS.filter(
    (transit) => transit.year >= startYear && transit.year <= endYear,
  ).map((transit) => ({
