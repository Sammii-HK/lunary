...
export default function TestStripePage() {
  ...
  const testStripe = async () => {
    ...
    try {
      ...
      if (!monthlyPriceId) {
        setResult('❌ NEXT_PUBLIC_STRIPE_MONTHLY_PRICE_ID not set in environment variables');
        return;
      }
    ...
    <div className="min-h-screen bg-zinc-900 p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-zinc-800 rounded-lg p-6 space-y-6">
          <div className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className={`${process.env.NEXT_PUBLIC_STRIPE_MONTHLY_PRICE_ID ? 'text-green-400' : 'text-red-400'}`}>
                  {process.env.NEXT_PUBLIC_STRIPE_MONTHLY_PRICE_ID ? '✅ Set' : '❌ Missing'}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-zinc-400">Yearly Price ID:</span>
                <span className={`${process.env.NEXT_PUBLIC_STRIPE_YEARLY_PRICE_ID ? 'text-green-400' : 'text-red-400'}`}>
                  {process.env.NEXT_PUBLIC_STRIPE_YEARLY_PRICE_ID ? '✅ Set' : '❌ Missing'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-semibold text-white">Test Checkout Creation</h2>
            <button
              onClick={testStripe}
              disabled={loading}
