  ...
  return (
    <div className='min-h-screen bg-surface-base text-content-primary flex flex-col'>
      ...
      <section className='max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-16 w-full'>
        <div className='max-w-2xl'>
          <div className='flex items-center gap-3 mb-6'>
            <span className='text-xs font-medium px-3 py-1 rounded-full bg-layer-base/50 text-content-brand border border-lunary-primary-800'>
              iOS App
            </span>
          ...
          <h1 className='text-4xl md:text-6xl font-semibold text-content-primary mb-5 leading-tight'>
            Ace your next
            <br />
            <span className='text-lunary-primary-400'>interview</span>
          </h1>
          <p className='text-content-muted text-lg md:text-xl leading-relaxed mb-8 max-w-lg'>
            Practice out loud. Get AI feedback on every answer. Track what you
            struggle with and fix it before the real thing.
          </p>
          <div className='flex flex-wrap items-center gap-4'>
            <span className='inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-stroke-default bg-surface-elevated text-content-muted font-semibold text-sm cursor-default'>
      ...
      <section className='max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full'>
        <h2 className='text-2xl md:text-3xl font-semibold text-content-primary mb-2'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5'>
          {features.map((f) => (
            <div
              key={f.title}
              className='p-5 rounded-2xl border border-stroke-subtle bg-surface-elevated/40'
            >
      ...
      {/* Privacy section */}
      <section className='max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full'>
        <div className='rounded-2xl border border-stroke-subtle bg-surface-elevated/30 p-8 md:p-10'>
          <div className='mb-8'>
            <p className='text-lunary-primary-400 text-sm font-medium uppercase tracking-widest mb-2'>
              Privacy first
            </p>
            <h2 className='text-2xl md:text-3xl font-semibold text-content-primary mb-3'>
              Your practice stays yours
            </h2>
            <p className='text-content-muted max-w-lg'>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-6'>
            {privacyPoints.map((p) => (
              <div key={p.title} className='flex gap-3'>
                <div className='mt-0.5 w-5 h-5 rounded-full bg-layer-base/60 border border-lunary-primary-700 flex-shrink-0 flex items-center justify-center'>
                  <svg
      ...
      <section className='max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full'>
        <div className='text-center'>
          ...
          <p className='text-content-muted mb-8'>
            Launching on the App Store soon. Free to start with a 14-day premium
            trial.
          </p>
          <span className='inline-flex items-center gap-2 px-8 py-3.5 rounded-xl border border-stroke-default bg-surface-elevated text-content-muted font-semibold text-sm cursor-default'>
            <svg className='w-5 h-5' viewBox='0 0 24 24' fill='currentColor'>
              <path d='M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z' />
            </svg>
            Coming soon to the App Store
      ...
      <div className='max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 w-full'>
        <div className='pt-8 border-t border-stroke-subtle flex flex-wrap gap-5'>
          <Link
