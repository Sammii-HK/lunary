...
import type { TourId, TourContext, FeatureTour } from '@/lib/feature-tours/tour-system';
...
export function useFeatureTour(context: TourContext) {
  ...
  const checkTours = useCallback(() => {
    const tour = FEATURE_TOURS.find((t) => {
      ...
      if (t.requiredTier && !t.requiredTier.includes(context.userTier)) return false;
      if (t.excludedTier && t.excludedTier.includes(context.userTier)) return false;
