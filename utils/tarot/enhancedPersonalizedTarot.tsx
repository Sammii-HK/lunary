...
const getDailyInfluences = (today: Date, userBirthday?: string) => {
  ...
  while (reducedUniversal > 9 && ![11, 22, 33].includes(reducedUniversal)) {
    reducedUniversal = reducedUniversal.toString().split('').reduce((sum, digit) => sum + parseInt(digit), 0);
  }
  
  // Calculate Personal Day Number if birthday available
  let personalDay = 1;
  if (userBirthday) {
    const [birthYear, birthMonth, birthDay] = userBirthday.split('-').map(Number);
    const currentYear = today.getFullYear();
    const currentMonth = today.getMonth() + 1;
    const currentDay = today.getDate();
    
    const personalSum = birthMonth + birthDay + currentYear + currentMonth + currentDay;
    personalDay = personalSum;
    while (personalDay > 9 && ![11, 22, 33].includes(personalDay)) {
      personalDay = personalDay.toString().split('').reduce((sum, digit) => sum + parseInt(digit), 0);
    }
  }
...
const getEnhancedPersonalCard = (userName?: string, userBirthday?: string): { card: TarotCard; reason: string } => {
  ...
  const numerologyInfluence = dailyInfluences.universalDay + dailyInfluences.personalDay;
  ...
  const planetMeaning = planetaryMeanings[planetaryInfluence as keyof typeof planetaryMeanings] || 'cosmic influence';
  const numberMeaning = numerologyMeanings[dailyInfluences.personalDay as keyof typeof numerologyMeanings] || 'numerical vibration';
...
const analyzeTarotTrends = (userName?: string, days: number = 30): TrendAnalysis => {
  ...
  pastReadings.forEach(reading => {
    cardFrequency[reading.card.name] = (cardFrequency[reading.card.name] || 0) + 1;
  ...
  if (frequentCards.length > 0) {
    ...
    patternInsights.push(`"${cardName}" has appeared ${count} times in ${days} days, suggesting this theme is particularly relevant to your current life path.`);
  }
  
  // Check for theme patterns
  if (keywordCounts['Love'] && keywordCounts['Love'] >= 5) {
    patternInsights.push('Love and relationship themes have been prominent, indicating a focus on heart matters and connections.');
  }
  
  if (keywordCounts['Transformation'] && keywordCounts['Transformation'] >= 3) {
    patternInsights.push('Transformation energies suggest you\'re in a significant period of personal growth and change.');
  }
  
  if (keywordCounts['Wisdom'] && keywordCounts['Wisdom'] >= 4) {
    patternInsights.push('Wisdom themes indicate you\'re being called to integrate important life lessons.');
  }
  ...
  Object.entries(weeklyPatterns).forEach(([day, cards]) => {
    ...
    const topKeyword = Object.entries(dayKeywords)
      .sort(([,a], [,b]) => b - a)[0];
    
    if (topKeyword && topKeyword[1] >= 2) {
      cyclicInfluences.push(`${dayNames[parseInt(day)]}s often bring themes of "${topKeyword[0]}", suggesting this day of the week carries special significance for you.`);
    }
  });
...
export const getEnhancedPersonalizedTarotReading = (
  ...
  const { card: personal, reason: personalCardReason } = getEnhancedPersonalCard(userName, userBirthday);
  ...
  const advice = generateEnhancedAdvice(daily, weekly, personal, userName, trendAnalysis);
