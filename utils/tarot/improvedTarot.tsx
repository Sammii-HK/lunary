...
type TrendAnalysis = {
  ...
  numberPatterns: Array<{ number: string; count: number; reading: string; cards: string[] }>;
...
const getCardDetails = (card: TarotCard): { keywords: string[], information: string } => {
...
const getNumberPatternReading = (number: string, count: number, timeFrame: number): string => {
...
const getArcanaReading = (type: string, count: number, total: number): string => {
  if (type === 'Major') {
...
const getSuitReading = (suit: string, count: number, total: number): string => {
  ...
  if (significanceRatio >= significanceThreshold) {
    switch (suit) {
      case 'Cups': return `Cups appearing strongly - Your emotional and relationship life needs major attention. Heart matters are central to your current journey.`;
      case 'Wands': return `Wands dominating your readings - Creative fire and action energy are prominent. Time for bold moves and passion projects.`;
      case 'Swords': return `Swords cutting through - Mental challenges and decisions are at the forefront. Your intellect is being tested and refined.`;
      case 'Pentacles': return `Pentacles grounding you - Material and practical matters require focused attention. Build solid foundations in the physical world.`;
      case 'Major Arcana': return `Major Arcana guiding strongly - Significant spiritual lessons and karmic experiences are unfolding in your life.`;
    }
  } else if (significanceRatio >= 1.2) {
    switch (suit) {
      case 'Cups': return `Cups emerging as a theme - Emotional healing and relationships need gentle attention.`;
      case 'Wands': return `Wands lighting up - Creative energy and enthusiasm are building. Channel this fire wisely.`;
      case 'Swords': return `Swords present - Mental clarity and communication are key themes in your current path.`;
      case 'Pentacles': return `Pentacles steady - Practical foundations and material security need strengthening.`;
      case 'Major Arcana': return `Major Arcana active - Important spiritual lessons are present in your journey.`;
    }
  }
  return '';
};
...
const analyzeTrends = (userName?: string, timeFrameDays: number = 30): TrendAnalysis => {
  ...
  pastReadings.forEach((card) => {
    ...
    suitCardBreakdown[suit][card.name] = (suitCardBreakdown[suit][card.name] || 0) + 1;
    ...
    const numberMatch = card.name.match(/\b(Ace|Two|Three|Four|Five|Six|Seven|Eight|Nine|Ten)\b/);
    if (numberMatch) {
  ...
  if (arcanaCounts.major > 0) {
    const reading = getArcanaReading('Major', arcanaCounts.major, pastReadings.length);
    if (reading) {
      arcanaPatterns.push({
        type: 'Major Arcana',
  ...
  if (arcanaCounts.minor > 0) {
    const reading = getArcanaReading('Minor', arcanaCounts.minor, pastReadings.length);
    if (reading) {
      arcanaPatterns.push({
        type: 'Minor Arcana',
