...
function generateTitle(metadata: PackMetadata, series: keyof typeof PACK_SERIES): string {
...
export function generatePricing(series: keyof typeof PACK_SERIES, edition: string): number {
  ...
  const basePrice = edition.includes('Premium') || edition.includes('Deluxe') 
    ? basePricing.premium 
    : basePricing.base;
...
export function generateMonthlyPack(year: number, month: number, category: string): PackNaming {
...
export function generateSeasonalPack(year: number, season: 'spring' | 'summer' | 'autumn' | 'winter', category: string): PackNaming {
  const metadata: PackMetadata = {
    category,
    difficulty: 'intermediate',
    year,
...
export function generateSpecialEventPack(specialEvent: string, category: string, difficulty: 'beginner' | 'intermediate' | 'advanced' | 'master' = 'advanced'): PackNaming {
  const metadata: PackMetadata = {
    category,
    difficulty,
