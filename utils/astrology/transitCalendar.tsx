...
export const getUpcomingTransits = (startDate: dayjs.Dayjs = dayjs()): TransitEvent[] => {
...
const getLunarPhaseEvents = (date: dayjs.Dayjs): TransitEvent[] => {
  ...
  if (Math.abs(phase - 0) < 0.02 || Math.abs(phase - 1) < 0.02) {
    events.push({
      ...
      planet: 'Moon',
      event: 'New Moon',
      description: 'Perfect time for new beginnings, setting intentions, and planting seeds for future growth.',
      significance: 'high',
      type: 'lunar_phase'
    });
    ...
    events.push({
      ...
      planet: 'Moon',
      event: 'First Quarter',
      description: 'Time to take action on your intentions, overcome challenges, and push forward.',
      significance: 'medium',
      type: 'lunar_phase'
    });
    ...
    events.push({
      ...
      planet: 'Moon',
      event: 'Full Moon',
      description: 'Peak energy for manifestation, completion, and releasing what no longer serves.',
      significance: 'high',
      type: 'lunar_phase'
    });
    ...
    events.push({
      ...
      planet: 'Moon',
      event: 'Last Quarter',
      description: 'Time for reflection, gratitude, and preparing for the next cycle.',
      significance: 'medium',
      type: 'lunar_phase'
    });
  }
...
const getPlanetarySignChanges = (date: dayjs.Dayjs, chart: AstroChartInformation[]): TransitEvent[] => {
  const events: TransitEvent[] = [];
...
const getRetrogradeEvents = (date: dayjs.Dayjs): TransitEvent[] => {
  ...
  if (dayOfYear % 88 === 0) {
    events.push({
      ...
      description: 'Review communication, technology, and travel plans. Perfect time for reflection and revision.',
      significance: 'high',
      type: 'retrograde'
    });
    ...
    events.push({
      ...
      planet: 'Mercury',
      event: 'Goes Direct',
      description: 'Communication and technology issues clear up. Time to move forward with plans.',
      significance: 'medium',
      type: 'direct'
    });
  }
  ...
  if (dayOfYear % 584 === 0) {
    events.push({
      ...
      planet: 'Venus',
      event: 'Goes Retrograde',
      description: 'Review relationships, values, and financial matters. Time for inner reflection on love.',
      significance: 'high',
      type: 'retrograde'
    });
  }
...
const getPersonalYearNumber = (birthDate: dayjs.Dayjs, currentDate: dayjs.Dayjs): number => {
  const currentYear = currentDate.year();
...
const getSolarReturnInsights_Text = (personalYear: number, daysTillReturn: number): string => {
  const yearInsights = {
  ...
  const baseInsight = yearInsights[personalYear as keyof typeof yearInsights] || "A year of personal growth and development.";
...
const getSolarReturnThemes = (personalYear: number): string[] => {
  ...
  return themes[personalYear as keyof typeof themes] || ["Growth", "Learning", "Development", "Progress"];
}; 