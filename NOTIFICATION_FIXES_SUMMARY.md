...
**File:** `src/hooks/useNotificationDeepLink.tsx` _(NEW FILE)_
...
export const useNotificationModals = create<NotificationModalState>((set) => ({
  ...
  closeRetrogradeModal: () => set({ retrogradeModal: { open: false } }),

  openIngressModal: (planet, sign) =>
    set({ ingressModal: { open: true, planet, sign } }),
  closeIngressModal: () => set({ ingressModal: { open: false } }),
